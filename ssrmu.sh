...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="https://qr.salty.ml/?url=${SSurl}"
...
ssr_link_qr(){
	...
	SSRQRcode="https://qr.salty.ml/?url=${SSRurl}"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/lemonsn/salty/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/lemonsn/salty/master/ssrmu.sh" && chmod +x ssrmu.sh
