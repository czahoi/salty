...
if [[ $python38_version == 3.8.* ]]; then
    ...
    wget https://www.python.org/ftp/python/$python38_version/Python-$python38_version.tgz
    sleep 1
    if [ ! -e "Python-$python38_version.tgz" ]; then
        echo "Download failed!"
        exit 0
    else
        tar -zxf Python-$python38_version.tgz
...
\rm /usr/bin/pip
echo "Add python3.8 as system default python version..."
ln -s /usr/local/python3/bin/python3.8 /usr/bin/python
echo "Add pip3.8 as system default python-pip version..."
